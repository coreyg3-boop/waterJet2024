...
const moveDir = (OLD_PATH) => {

  const NEW_PATH = "";

  const fs = require('fs-extra');

  fs.move(OLD_PATH, NEW_PATH, err => {
...
const createDir = (client, project, selectedFiles) => {
  ...
  const writeFiles =  (selectedFiles, secondAttempt) => {
    fs.exists(path.join('X:\\waterjetDashboard\\pending\\',  client, project), (exists) => {
      if (exists) {
        selectedFiles.forEach((item, index) => {
          console.log(item, index);
          try {
            fs.copyFileSync(item.path, path.join('X:\\waterjetDashboard\\pending\\', client, project, item.name));
...
export { dbStuff, createDir, moveDir, updateStatus, showProjects };
//dbStuff(crudOp);
